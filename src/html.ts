...
import { fmtTokens, fmtSubagents, fmtCacheTtl, fmtContextSurface, fmtOutcomes, fmtRelaySummary } from './report.js';
...
export function renderHtml(
  events: StoredEvent[],
  opts: {
    days: number; follow?: FollowRow[]; previousEvents?: StoredEvent[];
    categorize?: CategorizeSummary; relay?: RelaySummary; prSessions?: Set<string>;
  },
): string {
...
${fmtOutcomes(m) ? `<p class="muted">${esc(fmtOutcomes(m))}</p>` : ''}
